# Hammerstein essay — site version (v1 draft, post-stop-slop, 2026-04-15)

_Draft for Ray's review. Voice calibrated from Ray's "Von Hammerstein's Ghost" Medium article + "An Amateur Guide To Wargame Design." Three log entries pulled from this project's own `vault/hammerstein-log/observations.md` (local-only) plus the CatalogDNA Hammerstein Observations Log (with Ray's explicit permission 2026-04-15). Stop-slop pass applied in this version: em-dashes removed, passive voice converted to active, adverbs trimmed, binary contrasts reframed, throat-clearing cut. **DO NOT move to `src/content/` without Ray's sign-off on v1.**_
